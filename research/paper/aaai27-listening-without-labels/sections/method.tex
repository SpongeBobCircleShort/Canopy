...
anomaly is its Mahalanobis distance
...
every anomaly is honestly unknown --- flagging is unaffected. As verified
...
$\phi$ is pluggable. We compare an in-house forest-trained CNN's pre-classifier
feature against a frozen AudioSet-pretrained encoder (CNN14), evaluating each
under identical background-density and prototype machinery. Because Stage~1 and
Stage~2 are closed-form over embeddings, swapping $\phi$ is the only change
required to test a new representation.
