...
DEFAULT_DATASET = "forestprotection/forest-wild-fire-sound-dataset"
...
def build_kaggle_fire_rows(root: Path, *, dataset: str = DEFAULT_DATASET) -> list[ManifestRow]:
    rows = []
...
def write_kaggle_fire_manifest(root: Path, output: Path, *, dataset: str = DEFAULT_DATASET) -> list[ManifestRow]:
    rows = build_kaggle_fire_rows(root, dataset=dataset)
    if not rows:
...
def write_kaggle_fire_curation(rows: list[ManifestRow], output: Path) -> None:
...
def _source_recording_id(relative: Path) -> str:
    ...
    return "__".join(part for part in parts if part) or "forest_wildfire"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Download/import a Kaggle forest wildfire dataset into Canopy manifests.")
    ...
    parser.add_argument("--output", type=Path, default=Path("data/audio/manifests/kaggle_forest_wildfire_manifest.csv"))
    parser.add_argument("--curation-output", type=Path, default=Path("data/audio/curation/kaggle_forest_wildfire_curation.csv"))
    args = parser.parse_args()

    root = args.root or download_kaggle_dataset(args.dataset)
    rows = write_kaggle_fire_manifest(root, args.output, dataset=args.dataset)
    write_kaggle_fire_curation(rows, args.curation_output)
